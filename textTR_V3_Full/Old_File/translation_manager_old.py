...
class TranslationEngine:
    ...
    def __init__(self, gemini_api_key=None, gemini_model="gemini-2.5-flash", protection_patterns=None):
        ...
        self.last_used_engine = None  # ติดตาม engine ที่ใช้แปลล่าสุด
    ...
    def translate(self, text: str, source_lang: str = 'auto', target_lang: str = 'th', 
        ...
        # ชื่อของแต่ละ engine สำหรับแสดงผล
        engine_names = {
            self._try_gemini: f'Google Gemini AI ({self.gemini_model})',
            self._try_googletrans: 'Googletrans',
            self._try_deep_translator: 'Deep Translator',
            self._try_google_api: 'Google Translate API',
            self._simple_translate: 'Basic Dictionary'
        }
        ...
        for engine in self.engines:
            try:
                ...
                if result and result != text:
                    # บันทึก engine ที่ใช้สำเร็จ
                    self.last_used_engine = engine_names.get(engine, 'Unknown')
                    return result
            except Exception:
                continue
        
        # ถ้าแปลไม่ได้ให้ส่งกลับข้อความเดิม
        self.last_used_engine = None
        return text
    ...
    def get_last_used_engine(self) -> str:
        """ดึงชื่อ engine ที่ใช้แปลล่าสุด"""
        return self.last_used_engine or "ไม่ทราบ"
...
class TranslationData:
    ...
    def translate_line(self, line_index: int, translated_text: str, used_engine: str = '') -> bool:
        ...
        if 0 <= line_index < len(self.lines):
            ...
            self.lines[line_index]['used_engine'] = used_engine
            return True
        return False
...
class TranslationTab(BaseTabComponent):
    ...
    def __init__(self, parent: tk.Widget):
        ...
        self.variables = {
            ...
            # Engine selection
            'selected_engine': tk.StringVar(value='auto'),  # auto, gemini, googletrans, deep_translator, google_api
            # Gemini settings
    ...
    def _create_language_section(self) -> None:
        ...
        engine_combo.current(0)  # Default to auto
    ...
    def _create_gemini_section(self) -> None:
        ...
        model_combo = ttk.Combobox(
            ...
            width=18,
            state='readonly',
            values=['gemini-2.5-flash', 'gemini-2.5-pro', 'gemini-2.5-flash-lite', 'gemini-2.0-flash', 'gemini-1.5-pro', 'gemini-1.5-flash']
        )
...
• \\b\\w+_\\w+\\b       - คำที่มี underscore"""
    ...
    def test_text_protection(self) -> None:
        ...
        def run_test():
            ...
            try:
                # สร้าง TextProtector โดยตรง (ไม่ต้องพึ่ง Gemini)
                from ai_translator import TextProtector
                
                # ดึง patterns ที่เลือกจาก UI
                enabled_patterns = []
                for pattern_name, pattern_var in self.variables['protection_patterns'].items():
                    if pattern_var.get():
                        enabled_patterns.append(pattern_name)
                
                # สร้าง TextProtector และตั้งค่า patterns
    ...
    def update_grid_display(self) -> None:
        ...
        for line_data in page_lines:
            ...
            # ดึงชื่อ engine ที่ใช้แปล (ย่อให้สั้นลง)
            used_engine = line_data.get('used_engine', '')
            if used_engine:
                # ย่อชื่อ engine ให้สั้นลง
    ...
    def translate_selected_line(self) -> None:
        ...
        try:
            ...
            # อัปเดตข้อมูล พร้อม engine ที่ใช้
    ...
    def _translate_batch_thread(self, line_indices: List[int]) -> None:
        ...
        used_engines = set()  # เก็บ engine ที่ใช้ทั้งหมด
        ...
        try:
            for i, line_index in enumerate(line_indices):
                ...
                try:
                    ...
                    # เลือก engine ตามที่ผู้ใช้เลือก
                    if selected_engine == 'Gemini AI' or (selected_engine.startswith('auto') and self.translation_engine.is_gemini_available()):
                        if self.translation_engine.is_gemini_available():
                            self.update_protection_settings()
                            translated_text = self.translation_engine.gemini_translator.translate_text(
                    ...
                    if translated_text is None:
                        if selected_engine == 'Googletrans':
                            ...
                            # auto mode - ใช้ translate ที่จะลองหลาย engine
                            translated_text = self.translation_engine.translate(
                                line_data['original'], source_lang, target_lang, prompt_type, custom_prompt,
                                translate_only_after_separator, custom_separator
                            )
...
• โมดแปลเฉพาะหลัง '{separator}' เหมาะสำหรับไฟล์เกมหรือ config files"""
    ...
    def is_code_line(self, text: str) -> tuple:
        ...
        # รูปแบบที่บ่งบอกว่าเป็น code
        code_patterns = [
            # ฟังก์ชัน/เมธอด definitions
            ...
            # Variable assignments ที่เป็น code style
            ...
            # Numbers/Constants only
            (r'^\s*[\d\.\-]+\s*$', 'number only'),
            (r'^\s*0x[a-fA-F0-9]+\s*$', 'hex number'),
            
            # File paths (likely config)
            (r'^[/\\]?[\w\-]+([/\\][\w\-\.]+)+[/\\]?$', 'file path'),
            ...
            # Email addresses (might be code/config)
            (r'^\S+@\S+\.\S+$', 'email address'),
            
            # Programming decorators/annotations
        ...
        # ตรวจสอบอัตราส่วนของอักขระพิเศษ (code มักมีอักขระพิเศษเยอะ)
        special_chars = set('{}[]()<>+=*/%&|^!~;:@#$\\')
        ...
        # ตรวจสอบ camelCase หรือ snake_case ที่เป็นลักษณะ code
    ...
    def detect_and_skip_code_lines(self) -> None:
        ...
        # สร้าง dialog สำหรับตั้งค่าและดูตัวอย่าง
        dialog = tk.Toplevel(self.parent.winfo_toplevel())
        ...
        def scan_lines():
            ...
            # Get lines to scan based on scope
            scope = scope_var.get()
            lines_to_scan = []
        ...
        # Allow multiple selection in tree
